...
author: Taylor Reiter and Marian L. Schmidt
date: July 5th, 2017
output:
  html_document:
    theme: readable
...
This markdown file was taken (and modified) from the [ANGUS 2017 lesson material](https://angus.readthedocs.io/en/2017/rmarkdown_rnaseq.html)
...
We will use a pairwise comparison of the Salmon Transcripts Per Million (TPM) to understand how well our replicates replicated each other. 

We used a modified version of [gather-counts.py](https://github.com/ngs-docs/angus/blob/2017/_static/gather-counts.py) to gather TPM data (transcripts per million) in order to make a plot of these data.The modified script output a bunch of `.tpm` files with the following columns: 
| transcript | count |
|---|---|

We will use these data below.   
...
In this report we will use a ggplot2-esque package to perform "sniff tests" to make pairwise comparison plots to see how well the replicates actually replicate each other.
...
We can do the same thing for the mutant samples. These are highly correlated as well, and also replicate each other well.
